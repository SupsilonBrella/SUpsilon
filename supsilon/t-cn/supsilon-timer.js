...
function generateDeletionLink1() {
	...
	var html = '';
	html += '<div>復製下列代碼塊並放置在頁面或討論串中：</div>';
	html += '<div>';
	html += '<blockquote><strong>[[iframe http://www.brella.top/supsilon/t-cn/supsilon-timer.html?timestamp=' + timestamp.getTime() + '&type=' + type + ' style="width: 400px; height: 50px;"]]</strong></blockquote>';
	html += '</div>';
	html += '<div>';
	html += '<iframe src="http://www.brella.top/supsilon/t-cn/supsilon-timer.html?timestamp=' + timestamp.getTime() + '&type=' + type + '" style="width: 400px; height: 50px;"></iframe>';
	html += '</div>';
	$('#generated').html(html);
}
...
function initGenerators() {
	...
	html += '<div>';
	html += '計時器類型: ';
	html += '<input type="radio" name="type" value="0" checked> 刪除計時器 &mdash; ';
	html += '<input type="radio" name="type" value="1"> 封禁計時器 &mdash; ';
	html += '<input type="radio" name="type" value="2"> 通用/其他計時器';
	html += '</div>';
	html += '<div>';
	html += '時間節點: ';
	html += '<select id="gen1Month" name="月數">';
	...
	html += '<div>';
	html += '計時器類型: ';
	html += '<input type="radio" name="type2" value="0" checked> 刪除計時器 &mdash; ';
	html += '<input type="radio" name="type2" value="1"> 封禁計時器 &mdash; ';
	html += '<input type="radio" name="type2" value="2"> 通用/其他計時器';
	html += '</div>';
	html += '<div>';
	html += '時間節點: ';
	html += '<select id="gen2Day" name="天數">';
	for (i = 0; i < 31; i ++) {
		html += '<option value="' + i + '">' + i + '</option>';
	}
	html += '</select>';
	html += ' 天 ';
	html += '<select id="gen2Hour" name="小時數">';
	for (i = 0; i < 24; i ++) {
		html += '<option value="' + i + '">' + i + '</option>';
	}
	html += '</select>';
